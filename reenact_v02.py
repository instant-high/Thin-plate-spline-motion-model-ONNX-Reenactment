...
#parser.add_argument("--loop", action="store_true", help="Loop driving video") # not yet implemented

# target video:
parser.add_argument("--crop_scale", type=float, default=1.25, help='bbox size around aligned head target video') #1.5
...
parser.add_argument("--audio", action="store_true", help="Audio")
...
def reenact():
    ...
    # copy driving video to get same framerate as target / keep new audio in sync
    ...
    os.system('cls')
    print("Changed driving video from " + str(fps_driving) + " fps to " + str(fps_target) +" fps to match target video framerate") 
    ...
    for index in tqdm(range(total_frames)):
        ...
        # detect driving video face every frame:
        if args.tracking:
            coords = get_driving_bbox(detector, frame_driving)
